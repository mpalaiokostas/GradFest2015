...
%% SETTING PARAMETERS
  ...
  disp(output)                        % Print the name of the user
  disp('Analysis starting...')

%% PREPARING THE SUMMARY FILE TO SAVE THE RESULTS    
  finalresult=cell(10,7);        % Define the size of the file (rows,columns)
...
for f=1:10 % Begin the loop
  ...
  %% READING INPUT
    ...
    filename=strcat('tutorial-',num2str(f),'.dat');  % Name of the file to import
    data=dlmread(filename);                          % Save the contents of the file to the matrix "data"
    cd('..')                                         % Return to previous directory
 
 
  %% PLOTTING ... (see the helppage for "plot" for details!)
    ...
    figurename=strcat('f',num2str(f),'-plot1');
  ...
  %% FITTING A FUNCTION TO NON-LINEAR DATA
    ...
    figurename=strcat('f',num2str(f),'-plot2');
    resolution='-r300';
    cd('figures')
  ...
  %% COLLECTING ANALYSIS RESULTS
    finalresult{f,1}=filename;
    finalresult{f,2}=num2str(average);
    finalresult{f,3}=num2str(stdev);
    finalresult{f,4}=num2str(minValue);
    finalresult{f,5}=num2str(maxValue);
    finalresult{f,6}=num2str(percLarger);
    finalresult{f,7}=num2str(percSmaller);
